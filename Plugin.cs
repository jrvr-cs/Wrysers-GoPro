...
namespace WrysersGoPro
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        public void PlayerSpawned()
        {
            ...
            IsSteamVR = PlayFabAuthenticator.instance.platform.ToString().ToLower() == "steam";
            MakeCamera();
        }
        ...
        void MakeCamera()
        {
            ...
            for (int i = 0; i < go.transform.childCount; i++) //this is a horrible way to do it, but it wont let me gameobject.find it
            {
                var c = go.transform.GetChild(i);
                if (c.name == "Third Person Camera")
                {
                    for (int j = 0; j < c.childCount; j++)
                    {
                        var c1 = c.GetChild(j);
                        if (c1.name == "Shoulder Camera")
                            cam = c1.gameObject;
                    }
                }
            }
